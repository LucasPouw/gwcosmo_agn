...
class PixelatedGalaxyCatalogLikelihood(object):
    ...
    def pxD_GH0(self,H0,z,sigmaz,ra,dec,m,color,Lambda=0.):
        """
        ...
        tempnum,tempden = self.__pxD_GH0_internal(H0, sampz, sampm, sampra, sampdec, sampcolor, count, Lambda=Lambda)
    ...
    def likelihood(self,H0,Lambda=0.,complete_catalog=False):
        ...
        if not complete_catalog:
            ...
            if self.px_OmegaG < 0.999:
                self.pO = self.pO_DH0()
                #self.pDO = den ### alternative to calculating pDO directly below, but requires both px_OH0 and pD_OH0 to use dblquad (not quad) ###
        ...
        res,index_G = pixel_split(self.galra,self.galdec,self.catalog.nside) 
        ...
        for i,idx in enumerate(index_G):
            ...
            pxG, pDG = self.pxD_GH0(H0,z,sigmaz,ra,dec,m,color,Lambda=Lambda)
            
            pixels_G[:,i] = (pxG / pDG) * self.pG*(1./self.npix) + (self.pxB*self.catalog.skymap[idx] / (self.pDB*(1./self.npix))) * self.pB*(1./self.npix)
            
        sum_pixels_G = np.sum(pixels_G,axis=1)
        index = np.arange(self.npix)
        index_O = np.delete(index,index_G)
        sum_pixels_O = np.sum(self.catalog.skymap[index_O])*self.pxO/(self.pDO*(1./self.npix))*(1./self.npix)
