...
class skymap(object):
    ...
    def __init__(self, filename):
        ...
        """
        try:
            prob, header = hp.read_map(filename,field=[0,1,2,3],h=True,nest=True)
            self.prob = prob[0]
            self.distmu = prob[1]
    ...
    def marginalized_distance(self):
        ...
        dp_dr = [np.sum(self.prob * r**2 * self.distnorm * norm(self.distmu, self.distsigma).pdf(r)) for r in dl]
        return dl, dp_dr
